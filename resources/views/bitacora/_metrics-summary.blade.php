...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
    ...
    <div class="bg-surface-900/80 border border-surface-700/50 hover:border-surface-600 rounded-xl p-4 text-center transition-all duration-200">
        ...
        <p class="text-xs text-surface-500 mt-1.5 flex items-center justify-center gap-1">
            P&L Promedio
            <x-metric-tooltip title="P&L Promedio por Trade" formula="P&L total / Total trades" body="Cuánto ganas o pierdes en promedio por operación. El tuyo es ${{ number_format($metrics['avg_pnl'], 2) }}. Si es negativo, revisa tu RR o WR." />
        </p>
    </div>
    ...
    <div class="bg-surface-900/80 border border-surface-700/50 hover:border-surface-600 rounded-xl p-4 text-center transition-all duration-200">
        ...
        <p class="text-xs text-surface-500 mt-1.5 flex items-center justify-center gap-1">
            Racha actual / Mejor
            <x-metric-tooltip title="Racha de Ganadores" body="Trades ganadores consecutivos: racha actual {{ $metrics['current_streak'] }}, mejor racha histórica {{ $metrics['best_streak'] }}. Indica consistencia — pero no operes con exceso de confianza." />
        </p>
    </div>
