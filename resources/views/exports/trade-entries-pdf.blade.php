...
<head>
    <meta charset="utf-8">
    <title>Trading Journal — {{ $user->name }}</title>
    <style>
...
<body>
    ...
    <table class="trades">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $trade->opened_at->format('d/m/Y H:i') }}</td>
                <td>{{ $trade->closed_at ? $trade->closed_at->format('d/m/Y H:i') : '-' }}</td>
                ...
                <td>{{ $trade->fee ? '$' . number_format($trade->fee, 2) : '-' }}</td>
                <td>{{ $trade->source ?? '-' }}</td>
                <td>{{ $trade->status }}</td>
            </tr>
            @endforeach
